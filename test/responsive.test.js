...
const viewports = [
  ...
  { name: 'laptop-1024',  width: 1024, height: 768, mobile: false, expectFab: true,  expectMore: true  },
  { name: 'ipad-768',     width: 768,  height: 1024, mobile: true,  expectFab: true,  expectMore: true  },
...
(async () => {
  ...
  for (const v of viewports) {
    ...
    if (v.expectFab) {
      ...
      await t('Cards inside drawer fit the viewport width', async () => {
        const hasComments = await page.evaluate(() => document.querySelectorAll('.tdoc-margin-comment').length > 0);
        ...
        const overflow = await page.evaluate(() => {
          for (const c of document.querySelectorAll('.tdoc-margin-comment')) {
            ...
            if (r.right > window.innerWidth + 1) return r.right;
          }
          return 0;
        });
        if (overflow) throw new Error(`card right=${overflow} exceeds viewport ${v.width}`);
        // close
        await page.click('h1', { position: { x: 5, y: 5 } });
        await page.waitForTimeout(150);
      });
